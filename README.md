...
| `stations` | array | ≥ 2 entries, each with `km`, `name`, `code`, `plat`, `side` (`'L'`\|`'R'`\|`'I'`), `elev`, `lat`, `lon`. Optional per-station fields: `platforms` (1–6, defaults to 2 for `side: 'I'` else 1), `type` (`'halt'`\|`'station'`\|`'terminus'`\|`'junction'`, defaults to `'station'`), `style` (`'modern'`\|`'victorian'`\|`'rural'`, defaults to `'modern'`), `footbridge` (boolean, defaults to false). Halts skip the canopy. Terminus stations at the route end get a concrete buffer stop with red/white striped uprights across both running tracks. Junction stations get a 60 m branch-line stub (sleepers + rails) at 30° from the main tangent on the side opposite the platform. `platforms` counts above 2 add "wing" platforms with their own track (sleepers + segmented rails along the platform length) and `PLATFORM n` badges. `style` swaps the shelter: modern (default grey slab + slim metal posts), victorian (ornate dark-green canopy + brick back wall + cast-iron posts with brass finials), or rural (small wooden lean-to with timber posts and a tilted plank roof). `footbridge: true` adds a pedestrian overpass across both running tracks with parapet-railed staircases dropping to each primary platform — used on the bundled major stations (Leeds, Shipley, Keighley, Skipton, Ilkley). Every non-halt platform face also gets a ticket-machine kiosk automatically (no schema field needed). |
